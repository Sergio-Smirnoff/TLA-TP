...
static void _generatePrologue(void) {
	_output(0, "%s",
		...
		"    }\n"
		"\n"
		"	private static void updateStateInRange(char[] charArray, int rangeStart, int rangeEnd, Token tokenAtEndOfRange) {\n"
    	"	    for (int i = rangeStart; i < rangeEnd - 1; i++) {\n"
    	"	        manageState(charArray[i], null);\n"
    	"	    }\n"
    	"	    manageState(charArray[rangeEnd - 1], tokenAtEndOfRange);\n"
    	"	}\n"
		...
		"			 updateStateInRange(chars, i, tokenAndConsume.consumeIndex, tokenAndConsume.token);\n"
